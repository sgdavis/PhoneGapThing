...
public class SystemNotification extends Plugin {
    ...
    {
        note.setLatestEventInfo((Context) this.ctx, contentTitle, contentText, contentIntent);
        note.number = number;
	...
	Intent notificationIntent = new Intent((Context)this.ctx, MainActivity.class);
        notificationIntent.setAction(Intent.ACTION_MAIN);
        notificationIntent = notificationIntent.setFlags(Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
        contentIntent = PendingIntent.getActivity((Context)this.ctx, 0, notificationIntent, 0);
